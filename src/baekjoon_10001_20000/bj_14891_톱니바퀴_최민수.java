...
public class bj_14891_톱니바퀴_최민수 {

	static ArrayList<Integer> cog[] = new ArrayList[4];

	public static void main(String[] args) throws Exception {
		// 테스트 입력
		System.setIn(new FileInputStream("res/baekjoon/bj_input_14891"));
		...
		for (int i = 0; i < 4; i++) {
			...
			for (int j = 0; j < 8; j++) {
				cog[i].add(Integer.parseInt(s[j]));
			}
		}
		...
		for (int i = 0; i < k; i++) {
			...
			// 돌리는 함수
			if (clock == 1)
				spinClockwise(spin, 0);
			else
				spinCounterClockwise(spin, 0);
		}
